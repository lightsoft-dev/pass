...
| ⌘P 퀵 명령 (`@` 점프, `+branch` 워크트리) | 익스텐션 명령의 진입점 (일반 검색 + `>명령` 필터) |
...
└──────────────────────────────────────────────────────────────┘
┌─ Tier 3: UI 기여 ────────────────────────────────────────────┐
│  구현: 독립 HTML/CSS/JS 창 + JSON bridge (apiVersion 2)       │
│  보류: 세션 카드 배지·버튼, 상세 패널 내부 위젯                 │
└──────────────────────────────────────────────────────────────┘
```
...
⌘P 팔레트: 일반 검색은 세션/프로젝트와 함께 활성화된 익스텐션 명령도 찾는다. 입력이 `>`로
시작하면 익스텐션 명령만 검색한다 (기존 `@` 점프, `+branch`와 나란히). 프리픽스가 `/`가 아닌
이유: 슬래시는 에이전트 슬래시 명령(`/compact` 등)을 세션으로 보내는 데 이미 쓰인다 —
VS Code의 커맨드 팔레트 관례(`>`)를 따른다.
...
### Web UI 창 (apiVersion 2)

v2는 pass가 소유하는 일반 macOS 창 안에 익스텐션의 HTML/CSS/JS를 렌더링한다. 프로덕션
리소스는 익스텐션 폴더 안에서만 읽고 `pass-extension://` 전용 scheme으로 전달한다. CSP는
네트워크·frame·form 전송을 기본 차단하며 WebView는 비영구 website data store를 사용한다.
...
페이지에는 동결된 `window.pass` 객체만 주입된다:

```js
const snapshot = await pass.getSnapshot(); // session:read 필요
const unsubscribe = pass.on("attention.pending", event => render(event));
await pass.runAction("ping", { message: "bridge online" });
pass.closeWindow();
```

- `pass.on`: `windows[].subscriptions`에 선언되고 해당 `events:*` 권한이 있는 이벤트만 수신한다.
...
- **활성/비활성**: UserDefaults에 enabled 집합과 승인한 콘텐츠 fingerprint를 저장한다.
  manifest/HTML/JS/script 등 파일이 바뀌면 다음 Reload에서 자동 비활성화되어 재검토가 필요하다.
  파일 삭제 = 제거.
...
- **상단 런처**: 활성·유효한 익스텐션은 Pass 패널 최상단과 macOS 메뉴바의 Extensions 메뉴에
  표시한다. 수동 명령은 기존 `ExtensionRuntime` 경로로 실행하고, 이벤트 전용 익스텐션도 활성
  상태를 확인할 수 있게 이름을 노출한다.
...
- **앱 내 마켓**: 공개 목록·검색·상세 조회는 로그인 없이 Cloudflare Worker에 접근할 수 있다.
  내 목록, 설치, 게시·수정·삭제·신고·운영 작업은 기존 Pass 데스크톱 계정 토큰이 필요하다.
  D1에는 게시자, Git URL, 매니페스트 스냅샷, 카테고리·태그, 설치 수, 신고와 숨김 상태만 저장하며
  별도 웹 스토어는 두지 않는다.
