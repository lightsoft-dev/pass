...
> 상태: **기획 문서.** 아직 구현된 것은 없다. 이 문서는 "VS Code처럼 사용자가 원하는 기능을
> 직접 추가할 수 있는 확장 시스템 + AI로 익스텐션을 만들어주는 기능"의 설계와 마일스톤을 정리한다.
...
| `attention.resolved` | session | EventRouter (started/ended) |
| `session.created` | session, project | SessionStore.createSession |
| `session.ended` | name, project | reconcile에서 소멸 감지 |
| `session.finished` | lastMessage, session | Stop hook (finished와 구분: FYI 전용) |
| `project.added` | rootPath | ProjectStore.remember |

### 액션 카탈로그와 필요 권한

| 액션 | 권한 | 실행기 | 비고 |
|---|---|---|---|
| `script` | `run:script` | Shell | cwd=익스텐션 디렉토리, 페이로드는 argv+stdin(JSON) |
| `sendText` | `session:send` | ReplyInjector | bare shell 거부 로직 그대로 |
| `createSession` | `session:create` | SessionStore | projectRoot + agent + 초기 프롬프트 |
...
| 단계 | 내용 | 신규 코드 (추정) |
|---|---|---|
| **E0 스파이크** | 매니페스트 스키마 확정, 손으로 쓴 익스텐션 1개를 로드→규칙 1개 발화까지. FINDINGS 기록 | ~200 LOC |
| **E1 로더/스토어** | `ExtensionManifest`(Codable+검증), `ExtensionStore`, Settings › Extensions 탭 (목록/토글/리로드/에러 표시) | ~500 LOC |
| **E2 명령** | ⌘P `/명령` 라우팅, 템플릿 변수 확장, `ActionExecutor` (script/sendText/notify/openURL/createSession), 실행 로그 | ~450 LOC |
| **E3 이벤트 규칙** | `ExtensionRuntime` — EventRouter·SessionStore·ProjectStore 탭, 규칙 매칭(on/if), 액션 디스패치 | ~300 LOC |
