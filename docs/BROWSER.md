# M6 설계 — 내장 브라우저 + `passctl` CLI

> Status: **설계 (구현 전)**. 이 문서가 M6의 계약이다. 구현 전 반드시 S6 스파이크(§10)로
> 위험 가정을 검증한다 — S0(FINDINGS.md)과 같은 방식.
...
2. **`passctl` CLI** — 세션 안의 AI 에이전트가 `passctl browser open <url>` 한 줄로
...
1. **에이전트가 결과를 보여준다** — Claude가 `npm run dev`를 띄우고
   `passctl browser open http://localhost:5173` 실행 → 숨어 있던 패널이 (포커스를
   뺏지 않고) 떠오르고, 그 세션 터미널 오른쪽에 페이지가 열린다.
2. **사람이 옆에 두고 본다** — ⌘B로 브라우저 스플릿을 열고 ⌘L에 URL 입력.
   PR 리뷰 페이지를 옆에 두고 세션에 회신한다.
3. **에이전트 자가 검증(v1.5)** — 에이전트가 `passctl browser screenshot -o /tmp/ui.png`
...
agent (tmux, PASS_SESSION=pass-myapp)
   │  "$PASS_CTL" browser open http://localhost:5173
   ▼
passctl ──POST /cli/browser/open──►  HookServer (127.0.0.1:49817)
...
## 5. Part B — `passctl` CLI

### 5.1 이름과 형태

- 바이너리 이름 **`passctl`**. `pass`는 유닉스 패스워드 매니저(`brew install pass`)와
  충돌하므로 쓰지 않는다. `launchctl`/`systemctl` 계열의 관례를 따른다.
- **Swift 실행 파일 타깃 `PassCtl`** (`Sources/PassCtl/`), 의존성은
  `swift-argument-parser` 하나. `--help`가 곧 에이전트용 문서이므로 서브커맨드/플래그
  도움말 품질이 중요하다 — 셸 스크립트+curl로는 부족.
- 앱 번들에 내장: `Pass.app/Contents/MacOS/passctl` (project.yml `copyFiles`,
  destination executables, Pass와 같은 Apple Development 서명 — BUILD.md 서명 규칙 준수).

### 5.2 배포와 발견 — 에이전트가 이 도구를 어떻게 아는가

설치 없는 경로를 기본으로, 세 겹:

1. **안정 경로 심링크** — 앱이 실행될 때마다 `~/.pass/bin/passctl` →
   `<현재 번들>/Contents/MacOS/passctl` 심링크를 갱신한다. 앱을 옮겨도/빌드 산출물로
   실행해도 항상 유효한 고정 경로가 생긴다.
2. **세션 환경변수** — 세션 생성 시 `tmux new-session -e PASS_CTL=~/.pass/bin/passctl`
   ...
   `"$PASS_CTL" browser open …`으로 호출하면 항상 안전.
...
3. **SessionStart advertise 훅 (발견의 핵심)** — ClaudeHooksInstaller에 **command 타입**
   ...
   { "hooks": { "SessionStart": [ { "hooks": [
       { "type": "command", "command": "$HOME/.pass/bin/passctl advertise", "timeout": 5 }
   ] } ] } }
   ```

   `passctl advertise`는 `PASS_SESSION`이 없거나 pass가 안 떠 있으면 **아무것도 출력하지
   ...
   > `"$PASS_CTL" browser open <url>` (e.g. your dev server). Capture what they see with
   > `"$PASS_CTL" browser screenshot -o <path>` and Read the file to verify your UI work.

   주의: S0에서 **HTTP 타입** SessionStart는 발화하지 않음이 확인됐다(FINDINGS §1) —
   command 타입은 별개 경로라 동작이 기대되지만, **S6.4에서 실측 필수**.
4. (선택) Settings › "Install CLI" — `/usr/local/bin/passctl` 심링크(관리자 권한).
   pass 밖 일반 터미널에서 사람이 쓸 때용.

### 5.3 명령 집합

```
passctl browser open <url> [--session <name>] [--background] [--json]
passctl browser close [--session <name>] [--json]
passctl browser tabs [--json]
passctl browser screenshot [-o <path>] [--session <name>] [--json]   # v1.5
passctl browser read [--format text|html] [--session <name>]         # v1.5
passctl status [--json]        # pass 실행 여부 + 버전 + 포트
passctl advertise              # SessionStart 훅 전용 (§5.2)
```
...
(ShareHandlers와 동형; PassCtl 타깃은 프레임워크 공유 없이 구조체를 미러 복사 —
...
POST /cli/browser/open
  ...
  → { "ok": false, "error": "unknown session 'pass-x' — passctl browser tabs로 확인" }
...
- `screenshot`: `path` 생략 시 pass가 `~/.pass/screenshots/<session>-<ts>.png`에 쓰고
  경로를 돌려준다 → CLI는 경로만 stdout에 출력(`open $(passctl browser screenshot)` 합성 가능).
  상대 경로는 CLI가 절대화해서 보낸다. 뷰포트 캡처(`takeSnapshot`); 전체 페이지는
  `createPDF` 기반 `passctl browser pdf`로 v2 후보.
...
- **권한 상승 없음이 핵심 논거.** pass 세션의 에이전트는 이미 사용자 권한의 비샌드박스
  ...
  `passctl`은 새 능력을 부여하는 게 아니라 **의도를 구조화**한다(임의 AppleScript 대신
...
| `Sources/PassCtl/…` | CLI 타깃(main + Browser/Status/Advertise 커맨드) |
...
| `Core/Constants.swift` | `ctlEnvVar = "PASS_CTL"`, `ctlSymlinkPath`, 상한 상수 |
| `Core/TmuxClient.swift` | `new-session`/`adoptTag`에 `PASS_CTL` 주입 (§5.2-2) |
...
| `project.yml` | `PassCtl` tool 타깃 + swift-argument-parser + copyFiles 임베드 |

### 마일스톤

- **M6.1 브라우저 패널** — BrowserStore/WebViewPool/워크스페이스 스플릿, ⌘B·⌘L·⌘⇧B,
  URL 정규화, 영속화. (사람이 손으로 쓰는 브라우저 완성)
- **M6.2 CLI 관제 평면** — PassCtl 타깃, `/cli/browser/open|close|tabs`, `status`,
  심링크·PASS_CTL 주입, 표면화 규칙. (에이전트가 열 수 있음)
...
- PassCtl: 인자 파싱·세션 결정 순서·종료 코드(서버는 로컬 목 URLSession).
...
3. **S6.3 tool 타깃 임베드+서명** — XcodeGen copyFiles(executables)로 넣은 passctl이
...
5. **S6.5 어댑트 세션의 PASS_CTL** — `set-environment` 이후 새 프로세스에만 반영되는
