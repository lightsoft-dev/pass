...
final class SessionStore {
    ...
    /// Start a project tool (normally a feature's development server) in a visible, durable tmux
    /// session. Unlike agent sessions, the document-provided command is launched directly and the
    /// session is tagged `.shell`, so pass never mistakes it for an agent that accepts replies.
    @discardableResult
    func createCommandSession(projectDir: String, featureID: String, command: String) async -> String {
        let git = await resolveGit(projectDir)
        let projectRoot = git?.projectRoot ?? projectDir
        let repo = git?.repoName ?? URL(fileURLWithPath: projectRoot).lastPathComponent
        let base = "\(PassConfig.sessionPrefix)\(Slug.make(repo))--preview-\(Slug.make(featureID))"
